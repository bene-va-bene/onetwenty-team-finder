...
do $$ begin
 begin perform public.enqueue_contact('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',gen_random_uuid(),'Self'); raise exception 'FAIL self'; exception when others then if SQLERRM<>'This is your own listing.' then raise; end if; end;
...
do $$ declare a jsonb; b jsonb; begin
 a:=public.enqueue_contact('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','33333333-3333-4333-8333-333333333333','Hello');
 b:=public.enqueue_contact('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','33333333-3333-4333-8333-333333333333','Hello');
 if a<>b or a ? 'to' or a ? 'email' then raise exception 'FAIL idempotency/privacy'; end if;
 begin perform public.enqueue_contact('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',gen_random_uuid(),'Again'); raise exception 'FAIL rate limit'; exception when no_data_found then null; end;
end $$;
set local role anon;
...
do $$ declare j jsonb; begin
 j:=public.mail_claim();
 if j->>'to'<>'owner@example.invalid' or j->>'replyTo'<>'sender@example.invalid' then raise exception 'FAIL addresses'; end if;
...
set local role service_role;
do $$ begin
 if public.mail_claim() is not null then raise exception 'FAIL closed delivery'; end if;
end $$;
reset role;
...
rollback;
select 'PASS mail privacy, verified auth, rate limits, duplicate protection, reminder date, close, expiry, storage retry, cascades, account gate' as result;
